...
function Catalogo() {
  ...
  const leagueSlug = params.liga;
  const leagueId = useMemo(
    () => taxonomies.find((t) => t.type === "league" && t.slug === leagueSlug)?.id ?? null,
    [taxonomies, leagueSlug],
  );
  const clubSlug = params.time;
  const clubId = useMemo(
    () => taxonomies.find((t) => t.type === "club" && t.slug === clubSlug)?.id ?? null,
    [taxonomies, clubSlug],
  );
  const countrySlug = params.selecao;
  const countryId = useMemo(
    () => taxonomies.find((t) => t.type === "country" && t.slug === countrySlug)?.id ?? null,
    [taxonomies, countrySlug],
  );
  ...
  const filtered = useMemo(() => {
    let list = products.filter((p) => {
      ...
      if (leagueId && p.league_id !== leagueId) return false;
      if (clubId && p.club_id !== clubId) return false;
      if (countryId && p.country_id !== countryId) return false;
    ...
    return list;
  }, [products, categoryId, leagueId, clubId, countryId, selected, size, availability, maxPrice, query, sort]);
